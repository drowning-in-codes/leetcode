...
- 困难：基本计算器（Basic Calculator）
- 中等：基本计算器 II（Basic Calculator II）
- 困难：柱状图中最大的矩形（Largest Rectangle in Histogram）
- 困难：接雨水（Trapping Rain Water）
- 中等：移掉 K 位数字（Remove K Digits）
- 中等：字符串解码（Decode String）
...
- 中等：单词拆分（Word Break）
- 中等：解码方法（Decode Ways）
- 困难：矩阵中的最长递增路径（Longest Increasing Path in a Matrix）
- 中等：不同的二叉搜索树（Unique Binary Search Trees）
- 中等：打家劫舍 III（House Robber III）

## 20. 二叉树进阶补充
- 困难：二叉树的序列化与反序列化（Serialize and Deserialize Binary Tree）
- 中等：二叉树的最近公共祖先（LCA）
- 中等：二叉搜索树的最近公共祖先（LCA）
- 中等：从前序与中序遍历序列构造二叉树
- 中等：二叉树展开为链表
...
## 22. 计算题（表达式与数值运算）
- 困难：基本计算器（Basic Calculator）
- 中等：逆波兰表达式求值（Evaluate Reverse Polish Notation）
- 中等：基本计算器 II（Basic Calculator II）
- 困难：基本计算器 III（Basic Calculator III）
- 中等：Pow(x, n)
- 中等：字符串相乘（Multiply Strings）
- 中等：分数到小数（Fraction to Recurring Decimal）
- 简单：字符串相加（Add Strings）
...
- 树：层序遍历、最大路径和、验证 BST
- 树进阶：二叉树序列化与反序列化、最近公共祖先（LCA）
- 树构建/变形：前序+中序构造二叉树、二叉树展开为链表
- 树遍历：递归前序、递归中序、递归后序
- 图：岛屿数量、课程表、最短路径
- DP：爬楼梯、打家劫舍、零钱兑换、LIS、股票问题
- 记忆化搜索：单词拆分、解码方法、矩阵最长递增路径、不同 BST、打家劫舍 III
- 回溯：全排列、子集、组合总和、N 皇后
- 贪心：跳跃游戏、区间调度
- 位运算：Single Number、Counting Bits
- 栈专题：基本计算器、基本计算器 II、柱状图最大矩形
- 栈专题扩展：接雨水、移掉 K 位数字、字符串解码
- 排序：快速排序、归并排序、堆排序
- 计算题：基本计算器、逆波兰表达式求值、基本计算器 II、基本计算器 III、Pow(x, n)、字符串相乘、分数到小数、字符串相加
